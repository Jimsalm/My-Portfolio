...
import { passwordSettingsSchema } from "@/features/settings/schemas";

export async function PUT(request: Request) {
  const limited = rateLimitRequest(request, { limit: 12 });
  ...
  try {
    const parsedInput = passwordSettingsSchema.parse(await parseJson(request));
    const data = await fetchAction(api.api.settings.updatePassword, {
